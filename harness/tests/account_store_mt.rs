use {
    ...
    mollusk_svm_result::ProgramResult,
...
fn test_transfer_with_context_mt() {
    ...
    let (result, _tc) = context.process_instruction_log(
        &solana_system_interface::instruction::transfer(&sender, &recipient, transfer_amount),
        Some(log.clone()),
        false,
    );
    println!("logs: {:?}", result.program_result);
...
fn test_multiple_transfers_with_persistent_state_mt() {
    ...
    let (result1, _tc1) = context.process_instruction_log(&instruction1, None, false);
    assert!(!result1.program_result.is_err());
    assert_eq!(result1.compute_units_consumed, DEFAULT_COMPUTE_UNITS);

    // Second transfer: Bob -> Charlie
    let instruction2 =
        solana_system_interface::instruction::transfer(&bob, &charlie, transfer2_amount);
    let (result2, _tc2) = context.process_instruction_log(&instruction2, None, false);
    assert!(!result2.program_result.is_err());
    assert_eq!(result2.compute_units_consumed, DEFAULT_COMPUTE_UNITS);
...
fn test_account_store_default_account_mt() {
    ...
    let (result, _tc) = context.process_instruction_log(&instruction, None, false);
    assert!(result.program_result.is_err());
    if let Err(e) = result.raw_result {
        assert_eq!(e, solana_instruction_error::InstructionError::Custom(SystemError::ResultWithNegativeLamports as u32));
    }
}
...
fn test_get_sysvar_mt() {
    ...
    let clock: Clock = mollusk.sysvars.clock.clone();
    assert_eq!(clock.slot, 0); // Default slot should be 0

    // Test getting epoch schedule sysvar
    let epoch_schedule: EpochSchedule = mollusk.sysvars.epoch_schedule.clone();
    assert!(epoch_schedule.slots_per_epoch > 0);

    // Test getting rent sysvar
    let rent: Rent = mollusk.sysvars.rent.clone();
...
fn test_set_sysvar_mt() {
    ...
    let mut new_clock: Clock = mollusk.sysvars.clock.clone();
    new_clock.slot = 42;
    new_clock.epoch = 1;
    mollusk.sysvars.clock = new_clock;

    // Verify the clock was updated
    let updated_clock: Clock = mollusk.sysvars.clock.clone();
    assert_eq!(updated_clock.slot, 42);
    assert_eq!(updated_clock.epoch, 1);

    // Test setting rent sysvar
    let mut new_rent: Rent = mollusk.sysvars.rent.clone();
    let original_lamports_per_byte = new_rent.lamports_per_byte_year;
    new_rent.lamports_per_byte_year = 12345;
    mollusk.sysvars.rent = new_rent;

    // Verify the rent was updated
    let updated_rent: Rent = mollusk.sysvars.rent.clone();
